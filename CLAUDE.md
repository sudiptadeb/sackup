...
**Backup on connect:** `MainActivity` has an intent-filter for `USB_DEVICE_ATTACHED` (mass-storage filter in `res/xml/usb_device_filter.xml`), so plugging in the drive launches the app; `handleDriveArrival()` polls the drive check until the volume mounts, then either prompts (`ui/DriveConnectedDialog.kt`), starts a run, or does nothing per `ConnectPrefs` (`"on_connect_mode"`: ASK default / AUTO / OFF). `DriveMountedReceiver` is a manifest receiver for `MEDIA_MOUNTED` (exempt from implicit-broadcast limits) that only posts a "USB drive connected" notification when the app is in the background — it never starts the service, since Android 12+ forbids foreground-service starts from a background broadcast. A run is a queue of group ids (`BackupService.start(context, groupIds, uri)`); `BackupProgress.groupIndex/groupCount` and the `run*` totals describe the queue. While running, the service listens for `MEDIA_UNMOUNTED/EJECT/REMOVED/BAD_REMOVAL` matching the drive volume (`util/DriveVolumes.kt`) and ends the run with outcome `INTERRUPTED`; `service/InterruptedBackups.kt` keeps the ids still needing a run in prefs so the app can offer "Continue".
...
- On connect the app asks before backing up (setting: ask / automatic / off); a run always covers every group, in order
- An unplugged drive ends the run as INTERRUPTED (never ERROR) and is resumable; the manifest is still rebuilt from verified files
